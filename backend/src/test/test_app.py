...
def test_create_user_success(client_override, session):
    ...
    new_user = session.scalar(select(User).where(User.email == test_user["email"]))
...
def test_login_user_success(client_override, session, add_user_database):
    ...
    existing_user = session.scalar(select(User).where(User.email == test_user["email"]))
...
def test_login_user_incorrect_password(client_override, session, add_user_database):
    response = client_override.post(
